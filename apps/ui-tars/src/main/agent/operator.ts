...
export class NutJSElectronOperator extends NutJSOperator {
  ...
  async execute(params: ExecuteParams): Promise<ExecuteOutput> {
    // Hide the prediction marker overlay before executing the action
    // to prevent the transparent overlay window from blocking mouse clicks.
    hideOverlay();

    // Small delay to ensure window is fully hidden before action
    await sleep(50);
    ...
    try {
      ...
      // Restore the overlay marker (it will also be recreated by next handleData).
      showOverlay();
    }
  }
}
